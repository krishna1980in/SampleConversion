...
namespace EmployeeManagementSystem
{
    ...
    partial class rss : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string strWebshopName = Server.HtmlEncode(Conversions.ToString(GetGlobalResourceObject("Kartris", "Config_Webshopname")));
            int intLanguageID = CkartrisBLL.GetLanguageIDfromSession;
            string strSkinFolder = LanguagesBLL.GetTheme(intLanguageID);
            int intLanguage = Conversions.ToInteger(Session["LANG"]);
            ...
            using (var xml = new XmlTextWriter(Response.OutputStream, Encoding.UTF8))
            {
                ...
                foreach (DataRow row in tblLatestNews.Rows)
                {
                    ...
                    xml.WriteElementString("pubDate", dateRFC822(Conversions.ToDate(row["N_DateCreated"])));
                    xml.WriteEndElement();
                }
