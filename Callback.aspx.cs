            ...
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(strGatewayName))
                {
                    ...
                    if (clsPlugin.CallbackSuccessful)
                    {
                        ...
                        {
                            if (Conversions.ToBoolean(Operators.ConditionalCompareObjectEqual(tblOrder.Rows[0]["O_Paid"], 0, false)) || intCallbackStep == 2)
                            {
                                // Store the order details
                                O_CouponCode = FixNullFromDB(tblOrder.Rows[0]["O_CouponCode"]);
                                O_TotalPriceGateway = Conversions.ToDouble(tblOrder.Rows[0]["O_TotalPriceGateway"]);
                                O_WLID = Conversions.ToInteger(tblOrder.Rows[0]["O_WishListID"]);
                                O_CustomerID = Conversions.ToInteger(tblOrder.Rows[0]["O_CustomerID"]);
                                O_LanguageID = Conversions.ToInteger(tblOrder.Rows[0]["O_LanguageID"]);
                                strBasketBLL = Conversions.ToString(tblOrder.Rows[0]["O_Status"]);
                                strBodyText = Conversions.ToString(tblOrder.Rows[0]["O_Details"]);
                                O_CurrencyIDGateway = Conversions.ToInteger(tblOrder.Rows[0]["O_CurrencyIDGateway"]);
                            }
                            else
                            {
