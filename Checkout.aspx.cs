...
#error Cannot convert ClassBlockSyntax - see comment for details
    /* Cannot convert ClassBlockSyntax, CONVERSION ERROR: Object reference not set to an instance of an object. in 'Partial Class _Checkout
     In...' at character 1056
    ...
    Partial Class _Checkout
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles Me.Load
            ...
            If Not Global.EmployeeManagementSystem.Page.IsPostBack Then
                ...
                'SET PAGE TITLE
                '---------------------------------------
                Global.EmployeeManagementSystem.Page.Title = GetLocalResourceObject("PageTitle_CheckOut") & " | " & Server.HtmlEncode(GetGlobalResourceObject("Kartris", "Config_Webshopname"))
                ...
                'Paypal, SagePay, etc.
                '=======================================
                Dim arrPaymentsMethods As String() = Global.Microsoft.VisualBasic.Strings.Split(strPaymentMethods, ",")
                ...
                If blnOrderIsFree Then
                    ...
                    ddlPaymentGateways.Items.Add(New Global.System.Web.UI.WebControls.ListItem("FREE", "po_offlinepayment" & "::False"))
                ...
                Else
                    ...
                    Try
                        ...
                        ddlPaymentGateways.Items.Add(New Global.System.Web.UI.WebControls.ListItem(GetGlobalResourceObject("Kartris", "ContentText_DropdownSelectDefault"), "::False"))
                        ...
                        For Each strGatewayEntry As String In arrPaymentsMethods
                            ...
                            Dim arrGateway As String() = Global.Microsoft.VisualBasic.Strings.Split(strGatewayEntry, "::")
                            ...
                            'based array).
                            '---------------------------------------
                            If Global.Microsoft.VisualBasic.Information.UBound(arrGateway) = 4 Then
                                ...
                                If arrGateway(4) = "p" Then
                                    ...
                                    If Global.Microsoft.VisualBasic.Strings.LCase(arrGateway(2)) = "true" Then
                                    ...
                                    '---------------------------------------
                                    If Global.Microsoft.VisualBasic.Strings.LCase(arrGateway(1)) = "test" Or Global.Microsoft.VisualBasic.Strings.LCase(arrGateway(1)) = "fake" Then
                                        blnOkToAdd = HttpSecureCookie.IsBackendAuthenticated

                                        'Gateway is turned off, don't add it
                                        'to the list.
                                    ElseIf Global.Microsoft.VisualBasic.Strings.LCase(arrGateway(1)) = "off" Then
                                        blnOkToAdd = False
                                    End If
                                Else
                                ...
                                If blnOkToAdd Then
                                    ...
                                    If Interfaces.Utils.TrimWhiteSpace(strFriendlyName) <> "" Then
                                        ddlPaymentGateways.Items.Add(New Global.System.Web.UI.WebControls.ListItem(strFriendlyName, arrGateway(0).ToString & "::" & arrGateway(3).ToString))
                                    Else
                                        ddlPaymentGateways.Items.Add(New Global.System.Web.UI.WebControls.ListItem(strGatewayName, arrGateway(0).ToString & "::" & arrGateway(3).ToString))
                                    End If
                            ...
                            Else
                                ...
                                'gateway config
                                Throw New Global.System.Exception("Invalid gatewaylist config setting!")
                            End If
                        ...
                        If ddlPaymentGateways.Items.Count = 1 Then
                            Throw New Global.System.Exception("No valid payment gateways")
                        ...
                        ElseIf ddlPaymentGateways.Items.Count = 2 Then
                            ...
                            Me._SelectedPaymentMethod = arrSelectedGateway(0)
                            Me._blnAnonymousCheckout = CBool(arrSelectedGateway(1))
                            ...
                            If Me._SelectedPaymentMethod = "PO_OfflinePayment" Then phdPONumber.Visible = True Else phdPONumber.Visible = False

                            'Store value in hidden field. We hope this will be more
                            'robust if page times out
                            litPaymentGatewayHidden.Text = Me._SelectedPaymentMethod
                        Else
                        ...
                        '---------------------------------------
                    Catch ex As Global.System.Exception
                        Throw New Global.System.Exception("Error loading payment gateway list")
                    End Try
                End If
            ...
            If Not IsNothing(CurrentLoggedUser) Then
                ...
                If Not Global.EmployeeManagementSystem.Page.IsPostBack Then
                    ...
                    Dim lstUsrAddresses As Global.System.Collections.Generic.List(Of KartrisClasses.Address) = Nothing
                    ...
                    If (Not CurrentLoggedUser.DefaultBillingAddressID = CurrentLoggedUser.DefaultShippingAddressID) Then
                        If Not Me._blnAnonymousCheckout Then
        ...
        ''' Page Load Complete
        ''' </summary>
        Protected Sub Page_LoadComplete(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles Me.LoadComplete
            'Fails for new users
        ...
        ''' </summary>
        Protected Sub ddlPaymentGateways_SelectedIndexChanged(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles ddlPaymentGateways.SelectedIndexChanged
            Dim arrSelectedGateway() As String = Split(ddlPaymentGateways.SelectedItem.Value, "::")
            Me._SelectedPaymentMethod = arrSelectedGateway(0)
            Me._blnAnonymousCheckout = CBool(arrSelectedGateway(1))
            Me.ConfigureAddressFields(True)
            ...
            If Me._SelectedPaymentMethod = "PO_OfflinePayment" Then
        ...
        Private Sub ConfigureAddressFields(Optional ByVal blnUpdateAddressPanel As Boolean = False)
            If Me._blnAnonymousCheckout Then
        ...
        ''' shipping methods
        ''' </summary>
        Protected Sub BillingCountryUpdated(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles UC_BillingAddress.CountryUpdated
            If chkSameShippingAsBilling.Checked Then
                pnlShippingAddress.Visible = False
                UC_ShippingAddress.Visible = False
                Me.RefreshShippingMethods("billing")
            Else
                pnlShippingAddress.Visible = True
                UC_ShippingAddress.Visible = True
                Me.RefreshShippingMethods("shipping")
            End If
            updAddresses.Update()
        End Sub
        ...
        ''' shipping methods
        ''' </summary>
        Protected Sub ShippingCountryUpdated(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles UC_ShippingAddress.CountryUpdated
            If chkSameShippingAsBilling.Checked Then
                pnlShippingAddress.Visible = False
                UC_ShippingAddress.Visible = False
                Me.RefreshShippingMethods("billing")
        ...
        ''' </summary>
        Protected Sub chkSameShippingAsBilling_CheckedChanged(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles chkSameShippingAsBilling.CheckedChanged
            If chkSameShippingAsBilling.Checked Then
                pnlShippingAddress.Visible = False
                UC_ShippingAddress.Visible = False
                Me.RefreshShippingMethods("billing")
        ...
        ''' to next stage
        ''' </summary>
        Protected Sub btnProceed_Click(ByVal sender As Object, ByVal e As Global.System.EventArgs) Handles btnProceed.Click
            If String.IsNullOrEmpty(Me._SelectedPaymentMethod) Then
                Dim arrSelectedGateway() As String = Split(ddlPaymentGateways.SelectedItem.Value, "::")
                Me._SelectedPaymentMethod = arrSelectedGateway(0)
                Me._blnAnonymousCheckout = CBool(arrSelectedGateway(1))
            End If
            ...
            Dim strGatewayName As String = Me._SelectedPaymentMethod
            ...
            If mvwCheckout.ActiveViewIndex = "1" Then
                ...
                '=======================================
                If Global.EmployeeManagementSystem.Page.IsValid And (UC_BasketView.SelectedShippingID <> 0 Or UC_BasketView.GetBasket.AllFreeShipping) And blnValidEUVAT Then
                    ...
                    If Not blnBasketAllDigital Then
                        If Not Me._blnAnonymousCheckout AndAlso UC_BillingAddress.SelectedAddress IsNot Nothing Then
                            If chkSameShippingAsBilling.Checked Then
                    ...
                    If Me._SelectedPaymentMethod.ToLower = "po_offlinepayment" Then
                    ...
                    Else
                        ...
                        Else
                            litPaymentMethod.Text = Me._SelectedPaymentMethod
                        End If
                    End If
                    ...
                    End If
                    Dim intGatewayCurrency As Global.System.Int16
                    ...
                    ElseIf clsPlugin.GatewayName.ToLower = "braintreepayment" Then  ' show BrainTree input form
                        ...
                        If Not clientToken.[Equals]("") Then
                            ...
                            Me,
                            GetType(Global.EmployeeManagementSystem.Page),
                            "BrainTreeSelected",
            ...
            ElseIf mvwCheckout.ActiveViewIndex = "2" Then
                ...
                If phdCreditCardInput.Visible Then
                    ...
                    Global.EmployeeManagementSystem.Page.Validate("CreditCard")
                ...
                clsPlugin.GatewayName.ToLower = "braintreepayment" Then
                    ...
                    Dim sbdHTMLOrderEmail As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder
                    Dim sbdHTMLOrderContents As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder
                    Dim sbdHTMLOrderBasket As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder

                    'Dim strBillingAddressText As String, strShippingAddressText As String
                    Dim strSubject As String = ""
                    Dim strTempEmailTextHolder As String = ""

                    Dim sbdNewCustomerEmailText As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder
                    Dim sbdBodyText As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder
                    Dim sbdBasketItems As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder
                    Dim arrBasketItems As Global.System.Collections.Generic.List(Of Kartris.BasketItem)
                    ...
                    If Global.System.Configuration.ConfigurationManager.AppSettings("TaxRegime").ToLower = "us" Or Global.System.Configuration.ConfigurationManager.AppSettings("TaxRegime").ToLower = "simple" Then
                    ...
                    If Not String.IsNullOrEmpty(objBasket.CouponName) And objBasket.CouponDiscount.IncTax = 0 Then
                        strTempEmailTextHolder = GetGlobalResourceObject("Email", "EmailText_OrderEmailBreaker") & Global.Microsoft.VisualBasic.Constants.vbCrLf & " " & GetGlobalResourceObject("Basket", "ContentText_ApplyCouponCode") & Global.Microsoft.VisualBasic.Constants.vbCrLf & " " & objBasket.CouponName & Global.Microsoft.VisualBasic.Constants.vbCrLf
                    ...
                    If objBasket.PromotionDiscount.IncTax < 0 Then
                        For Each objPromotion As PromotionBasketModifier In UC_BasketSummary.GetPromotionsDiscount
                            With objPromotion
                                ...
                                End If
                                If strPromotionDescription <> "" Then strPromotionDescription += Global.Microsoft.VisualBasic.Constants.vbCrLf & .Name Else strPromotionDescription += .Name
                            End With
                        Next
                    End If
                    ...
                    If blnAppPricesIncTax = False Or blnAppShowTaxDisplay Then
                        sbdBodyText.AppendLine(" " & GetGlobalResourceObject("Checkout", "ContentText_OrderValue") & " = " & CurrenciesBLL.FormatCurrencyPrice(CUR_ID, objBasket.FinalPriceExTax, , False) & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        sbdBodyText.Append(" " & GetGlobalResourceObject("Kartris", "ContentText_Tax") & " = " & CurrenciesBLL.FormatCurrencyPrice(CUR_ID, objBasket.FinalPriceTaxAmount, , False) &
                         IIf(blnAppUSmultistatetax, " (" & Global.System.Math.Round((objBasket.D_Tax * 100), 5) & "%)", "") & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    End If
                    ...
                    sbdBodyText.Append(" " & GetGlobalResourceObject("Basket", "ContentText_TotalInclusive") & " = " & CurrenciesBLL.FormatCurrencyPrice(CUR_ID, objBasket.FinalPriceIncTax, , False) &
                                   " (" & CurrenciesBLL.CurrencyCode(CUR_ID) & " - " &
                                    ...
                                    ")" & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    sbdBodyText.AppendLine(GetGlobalResourceObject("Email", "EmailText_OrderEmailBreaker"))
                    If blnUseHTMLOrderEmail Then
                        ...
                        If blnAppPricesIncTax = False Or blnAppShowTaxDisplay Then
                            ...
                            sbdHTMLOrderContents.Append(" " & GetGlobalResourceObject("Kartris", "ContentText_Tax") & " = " & CurrenciesBLL.FormatCurrencyPrice(CUR_ID, objBasket.FinalPriceTaxAmount, , False) &
                             IIf(blnAppUSmultistatetax, " (" & Global.System.Math.Round((objBasket.D_Tax * 100), 5) & "%)", "") & "<br/>")
                        End If
                    ...
                    If blnDifferentGatewayCurrency Then
                        ...
                        If clsPlugin.GatewayName.ToLower = "bitcoin" Then numGatewayTotalPrice = Global.System.Math.Round(numGatewayTotalPrice, 8)

                        sbdBodyText.AppendLine(" " & GetGlobalResourceObject("Email", "EmailText_ProcessCurrencyExp1") & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "ContentText_TotalInclusive") & " = " & CurrenciesBLL.FormatCurrencyPrice(intGatewayCurrency, numGatewayTotalPrice, , False) &
                                       " (" & CurrenciesBLL.CurrencyCode(intGatewayCurrency) & " - " &
                                         ...
                                         ")" & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        sbdBodyText.Append(GetGlobalResourceObject("Email", "EmailText_OrderEmailBreaker") & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    ...
                    If objBasket.TotalAmountSaved > 0 And KartSettingsManager.GetKartConfig("frontend.checkout.confirmation.showtotalsaved") = "y" Then
                        strTempEmailTextHolder = " " & GetGlobalResourceObject("Email", "EmailText_TotalSaved1") & CurrenciesBLL.FormatCurrencyPrice(CUR_ID, objBasket.TotalAmountSaved, , False) & GetGlobalResourceObject("Email", "EmailText_TotalSaved2") & Global.Microsoft.VisualBasic.Constants.vbCrLf
                        sbdBodyText.AppendLine(strTempEmailTextHolder)
                        sbdBodyText.Append(GetGlobalResourceObject("Email", "EmailText_OrderEmailBreaker"))
                        If blnUseHTMLOrderEmail Then
                            sbdHTMLOrderEmail.Replace("[totalsavedline]", strTempEmailTextHolder.Replace(Global.Microsoft.VisualBasic.Constants.vbCrLf, "<br/>"))
                        End If
                    Else
                    ...
                    sbdBodyText.Append(Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    With UC_BillingAddress.SelectedAddress
                        sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "EmailText_PurchaseContactDetails") & Global.Microsoft.VisualBasic.Constants.vbCrLf)

                        If Not Me._blnAnonymousCheckout Then
                            sbdBodyText.Append(" " & GetGlobalResourceObject("Address", "FormLabel_CardHolderName") & ": " & .FullName & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                             " " & GetGlobalResourceObject("Email", "EmailText_Company") & ": " & .Company & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                             IIf(Not String.IsNullOrEmpty(txtEUVAT.Text), " " & GetGlobalResourceObject("Invoice", "FormLabel_CardholderEUVatNum") & ": " & txtEUVAT.Text & Global.Microsoft.VisualBasic.Constants.vbCrLf, ""))
                        End If

                        sbdBodyText.Append(" " & GetGlobalResourceObject("Kartris", "FormLabel_EmailAddress") & ": " & UC_KartrisLogin.UserEmailAddress & Global.Microsoft.VisualBasic.Constants.vbCrLf)

                        If Not Me._blnAnonymousCheckout Then
                            sbdBodyText.Append(" " & GetGlobalResourceObject("Address", "FormLabel_Telephone") & ": " & .Phone & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        End If

                        sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "EmailText_Address") & ":" & Global.Microsoft.VisualBasic.Constants.vbCrLf)

                        If Not Me._blnAnonymousCheckout Then
                            sbdBodyText.Append(" " & .StreetAddress & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                            " " & .TownCity & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                            " " & .County & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                            " " & .Postcode & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                            " " & .Country.Name)
                        Else
                            sbdBodyText.Append(GetGlobalResourceObject("Kartris", "ContentText_NotApplicable"))
                        End If

                        sbdBodyText.Append(Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf & GetGlobalResourceObject("Email", "EmailText_OrderEmailBreaker") & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    ...
                    'Shipping info
                    sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "EmailText_ShippingDetails") & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    Dim strShippingAddressEmailText As String = ""

                    If Not blnBasketAllDigital Then
                        If chkSameShippingAsBilling.Checked Then
                            With UC_BillingAddress.SelectedAddress
                                strShippingAddressEmailText = " " & .FullName & Global.Microsoft.VisualBasic.Constants.vbCrLf & " " & .Company & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                          " " & .StreetAddress & Global.Microsoft.VisualBasic.Constants.vbCrLf & " " & .TownCity & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                          " " & .County & Global.Microsoft.VisualBasic.Constants.vbCrLf & " " & .Postcode & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                          " " & .Country.Name & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf
                    ...
                    Else
                        ...
                        strShippingAddressEmailText = GetGlobalResourceObject("Kartris", "ContentText_NotApplicable") & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf
                    ...
                    sbdBodyText.Append(strShippingAddressEmailText & GetGlobalResourceObject("Email", "EmailText_OrderEmailBreaker") & Global.Microsoft.VisualBasic.Constants.vbCrLf)

                    'Comments and additional info
                    If Trim(txtComments.Text) <> "" Then
                        strTempEmailTextHolder = " " & GetGlobalResourceObject("Email", "EmailText_Comments") & ": " & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                             " " & txtComments.Text & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf
                        sbdBodyText.Append(strTempEmailTextHolder)
                        If blnUseHTMLOrderEmail Then
                            sbdHTMLOrderEmail.Replace("[ordercomments]", Server.HtmlEncode(strTempEmailTextHolder).Replace(Global.Microsoft.VisualBasic.Constants.vbCrLf, "<br/>"))
                        End If
                    Else
                        sbdHTMLOrderEmail.Replace("[ordercomments]", "")
                    End If

                    sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "EmailText_OrderTime2") & ": " & CkartrisDisplayFunctions.NowOffset & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    sbdBodyText.Append(" " & GetGlobalResourceObject("Email", "EmailText_IPAddress") & ": " & CkartrisEnvironment.GetClientIPAddress() & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    sbdBodyText.Append(" " & Request.ServerVariables("HTTP_USER_AGENT") & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    If blnUseHTMLOrderEmail Then
                    ...
                    If KartSettingsManager.GetKartConfig("frontend.users.emailnewaccountdetails") = "y" And blnNewUser Then
                        ...
                        sbdNewCustomerEmailText.Append(GetGlobalResourceObject("Email", "EmailText_CustomerSignedUpHeader") & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        sbdNewCustomerEmailText.Append(GetGlobalResourceObject("Email", "EmailText_EmailAddress") & UC_KartrisLogin.UserEmailAddress & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        sbdNewCustomerEmailText.Append(GetGlobalResourceObject("Email", "EmailText_CustomerCode") & UC_KartrisLogin.UserPassword & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        ...
                        sbdNewCustomerEmailText.Replace("<br>", Global.Microsoft.VisualBasic.Constants.vbCrLf).Replace("<br />", Global.Microsoft.VisualBasic.Constants.vbCrLf)
                    End If
                    ...
                    If Not (arrBasketItems Is Nothing) Then
                        ...
                        If arrBasketItems.Count = 0 Then
                            CkartrisFormatErrors.LogError("Basket items were lost in the middle of Checkout! Customer redirected to main Basket page." & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                                      "Details: " & Global.Microsoft.VisualBasic.Constants.vbCrLf & "C_ID:" & IIf(User.Identity.IsAuthenticated, CurrentLoggedUser.ID, " New User") & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                                        "Payment Gateway: " & clsPlugin.GatewayName & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                        ...
                        For Each Item As Kartris.BasketItem In arrBasketItems
                            With Item
                                ...
                                Dim sbdOptionText As New Global.System.Text.StringBuilder("")
                                If Not String.IsNullOrEmpty(.OptionText) Then sbdOptionText.Append(Global.Microsoft.VisualBasic.Constants.vbCrLf & " " & .OptionText().Replace("<br>", Global.Microsoft.VisualBasic.Constants.vbCrLf & " ").Replace("<br />", Global.Microsoft.VisualBasic.Constants.vbCrLf & " "))
                                ...
                                If .CustomText <> "" AndAlso String.IsNullOrEmpty(strCustomControlName) Then
                                    ...
                                    strCustomText = " [ " & .CustomText & " ]" & Global.Microsoft.VisualBasic.Constants.vbCrLf
                    ...
                    If blnUseHTMLOrderEmail Then
                        ...
                        sbdHTMLOrderContents.Insert(0, "<table id=""orderitems""><thead><tr>" & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                                "<th class=""col1"">" & GetGlobalResourceObject("Kartris", "ContentText_Item") & "</th>" & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                                "<th class=""col2"">" & GetGlobalResourceObject("Kartris", "ContentText_Price") & "</th></thead><tbody>" & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                    ...
                    If Not blnBasketAllDigital Then
                        If UC_BasketSummary.ShippingDestinationID = 0 Or UC_BasketView.ShippingDestinationID = 0 Then
                            CkartrisFormatErrors.LogError("Basket was reset. Shipping info lost." & Global.Microsoft.VisualBasic.Constants.vbCrLf & "BasketView Shipping Destination ID: " &
                                                      UC_BasketView.ShippingDestinationID & Global.Microsoft.VisualBasic.Constants.vbCrLf & "BasketSummary Shipping Destination ID: " &
                    ...
                    O_ID = objOrdersBLL.Add(C_ID, UC_KartrisLogin.UserEmailAddress, UC_KartrisLogin.UserPassword, UC_BillingAddress.SelectedAddress,
                                             UC_ShippingAddress.SelectedAddress, chkSameShippingAsBilling.Checked, objBasket,
                                              arrBasketItems, Global.Microsoft.VisualBasic.Interaction.IIf(blnUseHTMLOrderEmail, sbdHTMLOrderEmail.ToString, sbdBodyText.ToString), clsPlugin.GatewayName, CInt(Session("LANG")), CUR_ID,
                    ...
                    If O_ID > 0 Then
                        ...
                        If Not User.Identity.IsAuthenticated Then
                            If Global.System.Web.Security.Membership.ValidateUser(UC_KartrisLogin.UserEmailAddress, UC_KartrisLogin.UserPassword) Then
                                Global.System.Web.Security.FormsAuthentication.SetAuthCookie(UC_KartrisLogin.UserEmailAddress, True)
                            End If
                        End If
                        Try
                            Dim KartrisUser As KartrisMemberShipUser = Global.System.Web.Security.Membership.GetUser(UC_KartrisLogin.UserEmailAddress)
                        ...
                        If chkMailingList.Checked Then
                            Dim ML_SignupDateTime, ML_ConfirmationDateTime As Global.System.DateTime
                            Dim blnSignupCustomer As Boolean = False
                            If objOrder.CustomerID > 0 Then
                                Dim tblCustomerData As Global.System.Data.DataTable = BasketBLL.GetCustomerData(objOrder.CustomerID)
                            ...
                            If blnSignupCustomer Then
                                ...
                                Else
                                    ...
                                    Dim sbdMLBodyText As Global.System.Text.StringBuilder = New Global.System.Text.StringBuilder()
                                    Dim strBodyText As String
                                    Dim strMailingListSignUpLink As String = WebShopURL() & "Default.aspx?id=" & objOrder.CustomerID & "&r=" & strRandomString

                                    sbdMLBodyText.Append(GetGlobalResourceObject("Kartris", "EmailText_NewsletterSignup") & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                                    sbdMLBodyText.Append(strMailingListSignUpLink & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf)
                        ...
                        clsPlugin.GatewayName.ToLower = "braintreepayment" Then
                                    ...
                                    Try
                                        ...
                                        If transactionId <> "" Then
                                            ...
                                            Try
                                                ...
                                                End If
                                            Catch ex As Global.System.Exception
                                                Global.System.Diagnostics.Debug.Print(ex.Message)
                                            End Try
                                            BasketBLL.DeleteBasket()
                                            Session("Basket") = Nothing
                            ...
                            If blnResult Then
                                ...
                                If clsPlugin.CallbackSuccessful Or
                                    ...
                                    '---------------------------------------
                                    Dim tblOrder As Global.System.Data.DataTable = objOrdersBLL.GetOrderByID(O_ID)
                                    ...
                                    If tblOrder.Rows.Count > 0 Then
                                        If tblOrder.Rows(0)("O_Sent") = 0 Then
                                            ...
                                            If clsPlugin.GatewayName.ToLower = "po_offlinepayment" Then
                                                ...
                                                strPOline += GetGlobalResourceObject("Invoice", "ContentText_PONumber") & ": " & O_PurchaseOrderNo & Global.Microsoft.VisualBasic.Constants.vbCrLf
                                                strPOline += Global.Microsoft.VisualBasic.Constants.vbCrLf

                                                If blnUseHTMLOrderEmail Then
                                                    strCallBodyText = CStr(tblOrder.Rows(0)("O_Details"))
                                                    strCallBodyText = strCallBodyText.Replace("[poofflinepaymentdetails]", strPOline.Replace(Global.Microsoft.VisualBasic.Constants.vbCrLf, "<br />"))
                                                Else
                                            ...
                                            If clsPlugin.GatewayName.ToLower = "bitcoin" Then
                                                ...
                                                strBitcoinline += Global.Microsoft.VisualBasic.Constants.vbCrLf

                                                If blnUseHTMLOrderEmail Then
                                                    strCallBodyText = strCallBodyText.Replace("[bitcoinpaymentdetails]", strBitcoinline)
                                                Else
                                                    strCallBodyText += strBitcoinline.Replace("<br/>", Global.Microsoft.VisualBasic.Constants.vbCrLf) & strCallBodyText
                                                End If
                                            ...
                                            If KartSettingsManager.GetKartConfig("frontend.checkout.ordertracking") <> "n" Then
                                                ...
                                                If Not blnUseHTMLOrderEmail Then
                                                    strCustomerEmailText = GetGlobalResourceObject("Email", "EmailText_OrderLookup") & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf & WebShopURL() & "Customer.aspx" & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf
                                                End If
                                            End If
                                            ...
                                            If Not blnUseHTMLOrderEmail Then
                                                strCustomerEmailText = GetGlobalResourceObject("Email", "EmailText_OrderReceived") & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                                            GetGlobalResourceObject("Kartris", "ContentText_OrderNumber") & ": " & O_ID & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf &
                                            ...
                                            If KartSettingsManager.GetKartConfig("frontend.orders.emailmerchant") <> "n" Then
                                                strStoreEmailText = GetGlobalResourceObject("Email", "EmailText_StoreEmailHeader") & Global.Microsoft.VisualBasic.Constants.vbCrLf & Global.Microsoft.VisualBasic.Constants.vbCrLf
                                                If Not blnUseHTMLOrderEmail Then
                                                    strStoreEmailText += GetGlobalResourceObject("Kartris", "ContentText_OrderNumber") & ": " & O_ID & Global.Microsoft.VisualBasic.Constants.vbCrLf & strCallBodyText
                                                Else
        ...
        Protected Sub txtEUVAT_AutoPostback() Handles txtEUVAT.TextChanged, chkSameShippingAsBilling.CheckedChanged
            ...
            If Not String.IsNullOrEmpty(txtEUVAT.Text) Then
                ...
                If Global.Microsoft.VisualBasic.Strings.Left(strEUVatNumber, 2).ToUpper = strThisUsersCountryCode.ToUpper Then
        ...
        Private Function CheckVATNumber(ByVal strISOCode As String, ByVal strVatNumber As String) As Boolean
            strVatNumber = Global.Microsoft.VisualBasic.Strings.Replace(Global.Microsoft.VisualBasic.Strings.Replace(strVatNumber, " ", ""), "-", "")
            strVatNumber = Global.Microsoft.VisualBasic.Strings.Replace(Global.Microsoft.VisualBasic.Strings.Replace(strVatNumber, ".", ""), ",", "")
            Dim strCountryCodeFromVatNumber As String = Global.Microsoft.VisualBasic.Strings.Left(strVatNumber, 2)
            Dim numVatNumberLength As Integer = Global.Microsoft.VisualBasic.Strings.Len(strVatNumber) - 2
            Dim blnPassed As Boolean = False
            Select Case strISOCode
                Case "AT" 'Austria
                    blnPassed = numVatNumberLength = 9 And
                          Global.Microsoft.VisualBasic.Strings.UCase(Global.Microsoft.VisualBasic.Strings.[Mid](strVatNumber, 3, 1)) = "U" And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 8)) And
                ...
                Case "BE", "BG" 'Belgium, Bulgaria
                    blnPassed = (numVatNumberLength = 9 Or numVatNumberLength = 10) And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 9)) And
                       strCountryCodeFromVatNumber = strISOCode
                ...
                Case "FR" 'France
                    blnPassed = numVatNumberLength = 11 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 9)) And
                       Global.Microsoft.VisualBasic.Strings.InStr(strVatNumber, "O") = 0 And
                       Global.Microsoft.VisualBasic.Strings.InStr(strVatNumber, "I") = 0 And
                       strCountryCodeFromVatNumber = strISOCode
                ...
                Case "IE" 'Ireland
                    blnPassed = numVatNumberLength = 8 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.[Mid](strVatNumber, 5, 4)) And
                       Not Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 1)) And
                       strCountryCodeFromVatNumber = strISOCode

                Case "IT", "LV", "HR" 'Italy, Latvia, Croatia
                    blnPassed = numVatNumberLength = 11 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 11)) And
                       strCountryCodeFromVatNumber = strISOCode

                Case "NL" 'Netherlands
                    blnPassed = numVatNumberLength = 12 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 2)) And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.[Mid](strVatNumber, 3, 8)) And
                       Global.Microsoft.VisualBasic.Strings.[Mid](strVatNumber, 12, 1) = "B" And
                       strCountryCodeFromVatNumber = strISOCode

                Case "ES" 'Spain
                    blnPassed = numVatNumberLength = 9 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.[Mid](strVatNumber, 4, 7)) And
                       strCountryCodeFromVatNumber = strISOCode

                Case "SE" 'Sweden
                    blnPassed = numVatNumberLength = 12 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 12)) And
                       strCountryCodeFromVatNumber = strISOCode
                ...
                Case "CZ" 'Czech Republic
                    If numVatNumberLength >= 11 And numVatNumberLength <= 13 Then
                        strVatNumber = Global.Microsoft.VisualBasic.Strings.[Mid](strVatNumber, 4)
                        numVatNumberLength = Global.Microsoft.VisualBasic.Strings.Len(strVatNumber)
                    End If
                    blnPassed = numVatNumberLength >= 8 And
                ...
                Case "PL", "SK" 'Poland, Slovak Republic
                    blnPassed = numVatNumberLength = 10 And
                       Global.Microsoft.VisualBasic.Information.IsNumeric(Global.Microsoft.VisualBasic.Strings.Right(strVatNumber, 10)) And
                       strCountryCodeFromVatNumber = strISOCode
